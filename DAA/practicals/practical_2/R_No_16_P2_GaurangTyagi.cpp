...
{
    std::fstream strm("./data.csv", std::ios::in);
...
{
    std::fstream strm(file, std::ios::app);
    std::vector<Coltype> schema = parseSchema(schemaStr);
...
void insertionSort(std::vector<std::vector<value>> &data, const std::string &key, const std::string &schemaStr)
{
    ...
    for (int i = 0; i < data.size(); i++)
    {
        ...
        {
            data[j + 1][sortKey] = data[j][sortKey];
            --j;
        }
